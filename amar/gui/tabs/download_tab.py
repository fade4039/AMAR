"""Download page - Apple HIG card layout."""
...
class DownloadTab(ttk.Frame):
    ...
    def _build_ui(self):
        # Page title
        ttk.Label(self, text="Download", style="Title1.TLabel").pack(anchor=tk.W, pady=(0, 16))

        # --- URL Card ---
        url_card = ttk.LabelFrame(self, text="Apple Music URL", padding=12)
        ...
        ttk.Button(input_row, text="Paste", command=self._paste_url, style="Secondary.TButton").pack(side=tk.LEFT, padx=(0, 4))
        ttk.Button(input_row, text="Clear", command=self._clear_url, style="Secondary.TButton").pack(side=tk.LEFT)

        self._detect_var = tk.StringVar(value="Enter a URL to begin")
        ttk.Label(url_card, textvariable=self._detect_var, style="CardSecondary.TLabel").pack(anchor=tk.W, pady=(8, 0))

        # --- Options Card ---
        opts_card = ttk.LabelFrame(self, text="Options", padding=12)
        opts_card.pack(fill=tk.X, pady=(0, 12))

        row1 = ttk.Frame(opts_card)
        row1.pack(fill=tk.X)

        self._info_only = tk.BooleanVar(value=False)
        ttk.Checkbutton(row1, text="Info only (no media files)", variable=self._info_only).pack(side=tk.LEFT, padx=(0, 20))
        ...
        # Storefronts
        sf_frame = ttk.Frame(opts_card)
        sf_frame.pack(fill=tk.X, pady=(8, 0))

        self._sf_mode = tk.StringVar(value="current")
        ttk.Radiobutton(sf_frame, text="Current storefront only", variable=self._sf_mode, value="current").pack(side=tk.LEFT, padx=(0, 16))
        ttk.Radiobutton(sf_frame, text="All storefronts (~130)", variable=self._sf_mode, value="all").pack(side=tk.LEFT)
        ...
        self._cancel_btn = ttk.Button(btn_frame, text="Cancel", command=self._cancel_download, style="Danger.TButton", state=tk.DISABLED)
        ...
        self._overall_progress = LabeledProgress(prog_card, "Overall Progress")
    ...
    def _on_url_change(self, *_):
        ...
        if parsed:
            self._detect_var.set(f"Detected: {parsed.content_type}  |  ID: {parsed.item_id}  |  Storefront: {parsed.storefront.upper()}")
        ...
        try:
            ...
            for idx, sf in enumerate(storefronts):
                if self._cancelled:
                    break
                pct = (idx / total) * 100
                self._app.schedule_on_gui(self._overall_progress.set_progress, pct, f"Storefront {idx + 1}/{total}: {sf.upper()} - {COUNTRY_CODES.get(sf, sf)}")

                try:
                    if parsed.content_type == "artist":
                        self._app.schedule_on_gui(self._current_progress.set_label, "Downloading artist assets...")
                        ...
                        if self._include_videos.get() and not self._cancelled:
                            try:
                                self._app.schedule_on_gui(self._current_progress.set_label, "Fetching music video list...")
                                ...
                                for vi, vid in enumerate(video_ids):
                                    ...
                                    self._app.schedule_on_gui(self._current_progress.set_progress, (vi / max(len(video_ids), 1)) * 100, f"Music video {vi + 1}/{len(video_ids)}")
                                    self._app.schedule_on_gui(self._file_progress.reset)
                                    try: await video_ripper.rip(vid, sf, info_only, log, progress_callback=file_cb)
                                    except Exception as e: log(f"Error downloading music video {vid}: {e}", "ERROR")
                            except TokenExpiredError: raise
                            except Exception as e: log(f"Error fetching music videos: {e}", "ERROR")

                        if self._include_albums.get() and not self._cancelled:
                            try:
                                self._app.schedule_on_gui(self._current_progress.set_label, "Fetching album list...")
                                ...
                                for ai, aid in enumerate(album_ids):
                                    if self._cancelled: break
                                    self._app.schedule_on_gui(self._current_progress.set_progress, (ai / max(len(album_ids), 1)) * 100, f"Album {ai + 1}/{len(album_ids)}")
                                    self._app.schedule_on_gui(self._file_progress.reset)
                                    try: await album_ripper.rip(aid, sf, info_only, log, progress_callback=file_cb)
                                    except Exception as e: log(f"Error downloading album {aid}: {e}", "ERROR")
                            except TokenExpiredError: raise
                            except Exception as e: log(f"Error fetching albums: {e}", "ERROR")

                    elif parsed.content_type == "album":
                        self._app.schedule_on_gui(self._current_progress.set_label, "Downloading album assets...")
                        self._app.schedule_on_gui(self._file_progress.reset)
                        await album_ripper.rip(parsed.item_id, sf, info_only, log, progress_callback=file_cb)

                    elif parsed.content_type == "music-video":
                        self._app.schedule_on_gui(self._current_progress.set_label, "Downloading music video assets...")
                        self._app.schedule_on_gui(self._file_progress.reset)
                        await video_ripper.rip(parsed.item_id, sf, info_only, log, progress_callback=file_cb)

                    elif parsed.content_type == "station":
                        self._app.schedule_on_gui(self._current_progress.set_label, "Downloading station assets...")
                        self._app.schedule_on_gui(self._file_progress.reset)
            ...
            self._app.schedule_on_gui(self._overall_progress.set_progress, 100, "Complete!")
